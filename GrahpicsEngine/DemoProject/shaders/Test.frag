...
layout (binding=4) uniform sampler2D ambientFactor;
layout (binding=5) uniform sampler2D diffuseFactor;
layout (binding=6) uniform sampler2D specularFactor;
...
vec3 getAmbient(in vec3 factor, in Light light);
vec3 getDiffuse(in vec3 factor, in vec3 position, in vec3 normal, in Light light);
vec3 getSpecular(in vec3 factor, in vec3 position, in vec3 normal, in Light light);
...
void main()
{
	...
	vec3 fragAmbientFactor = texture(ambientFactor,outputUV).rgb;
	vec3 fragDiffuseFactor = texture(diffuseFactor,outputUV).rgb;
	vec3 fragSpecularFactor = texture(specularFactor,outputUV).rgb;
	...
	{
		vec3 ambient = getAmbient(fragAmbientFactor, lights.light[i]);
		vec3 diffuse = getDiffuse(fragDiffuseFactor, fragPosition, fragNormal, lights.light[i]);
		vec3 specular = getSpecular(fragSpecularFactor, fragPosition, fragNormal, lights.light[i]);
	...
	vec3 I_Local = lights.globalAmbientColor + lightsResult;
...
vec3 getDiffuse(in vec3 factor, in vec3 position, in vec3 normal, in Light light)
{
	...
	return light.diffuse * diff * factor;
}
...
vec3 getSpecular(in vec3 factor, in vec3 position, in vec3 normal, in Light light)
{
	...
	return light.specular * spec * factor;
}
